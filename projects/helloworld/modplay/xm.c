...
#define SAFE_MALLOC(type, dest, a) \
  dest = (type)malloc(a); \
  if (dest == NULL) { \
    MODFILE_Free(xm); \
    return -2; \
...
int MODFILE_SetXM(u8 *xmfile, int xmlength, MODFILE *xm) {
  ...
  /* Load patterns */
  SAFE_MALLOC(MOD_Note **, xm->patterns, sizeof(MOD_Note*) * xm->nPatterns);
  SAFE_MALLOC(int *, xm->patternLengths, sizeof(int) * xm->nPatterns);
  ...
  for (i = 0; i < xm->nPatterns; i++) {
    ...
    SAFE_MALLOC(MOD_Note *, xm->patterns[i], xm->patternLengths[i] * xm->nChannels * sizeof(MOD_Note));
  ...
  SAFE_MALLOC(MOD_Sample *, xm->samples, xm->nSamples * sizeof(MOD_Sample));
  memset(xm->samples, 0, xm->nSamples * sizeof(MOD_Sample));
  /*  xm->instruments = malloc(xm->nInstruments * sizeof(MOD_Instrument));*/
  SAFE_MALLOC(MOD_Instrument *, xm->instruments, xm->nInstruments * sizeof(MOD_Instrument));
  ...
  for (i = 0; i < xm->nInstruments; i++) {
    ...
    if (samplesininstr > 0) {
      ...
      if ((xmfile[ofs + 233] & 1) && (xmfile[ofs + 225] > 0)) {
				...
				/* Envelope points */
				xm->instruments[i].envVolume.envPoints = (EnvPoint *)malloc(sizeof(EnvPoint) * (int)xm->instruments[i].envVolume.numPoints);
      ...
      if ((xmfile[ofs + 234] & 1) && (xmfile[ofs + 226] > 0)) {
				...
				/* Envelope points */
				xm->instruments[i].envPanning.envPoints = (EnvPoint *)malloc(sizeof(EnvPoint) * (int)xm->instruments[i].envPanning.numPoints);
