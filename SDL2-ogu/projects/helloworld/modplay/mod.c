...
int MODFILE_SetMOD(u8 *modfile, int modlength, MODFILE *mod) {
  ...
  SAFE_MALLOC(MOD_Instrument *, mod->instruments, mod->nInstruments * sizeof(MOD_Instrument));
  memset(mod->instruments, 0, mod->nInstruments * sizeof(MOD_Instrument));
  /*  mod->samples = malloc(mod->nSamples * sizeof(MOD_Sample));*/
  SAFE_MALLOC(MOD_Sample *, mod->samples, mod->nSamples * sizeof(MOD_Sample));
...
/*  for (i = mod->nPatterns = 0; i < mod->songlength; i++) {
  ...
  SAFE_MALLOC(MOD_Note **, mod->patterns, sizeof(MOD_Note*) * mod->nPatterns);
  SAFE_MALLOC(int *, mod->patternLengths, sizeof(int) * mod->nPatterns);
  ...
  for (i = 0; i < mod->nPatterns; i++) {
    ...
    SAFE_MALLOC(MOD_Note *, mod->patterns[i], sizeof(MOD_Note) * mod->nChannels * 64);
...
/*	    if (operand < 32)
  ...
  for (i = 0; i < mod->nInstruments; i++) {
    ...
    if (mod->samples[i].sampleInfo.length != 0) {
      ...
      SAFE_MALLOC(MOD_Sample *, mod->samples[i].sampleInfo.sampledata, mod->samples[i].sampleInfo.length);
